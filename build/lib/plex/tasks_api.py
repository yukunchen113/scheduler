...
def put_gtasklists(tasklist_id: str, tasklist_name:str, gtasklists: list[GTaskList]):
    ...
    for gtasklist in gtasklists:
        ...
        for subtask in gtasklist["subtasks"]:
            new_tasks.append({"title":subtask, "parent":parents[gtasklist["name"]]})
    ...
    for ctask in cur_tasks:
        if new_tasks := [
            ...
            and (
                ntask.get("parent", "") == ctask.get("parent", "") # these parents don't match since one is an id and one is a title
    ...
    parents = {i["title"]:i["id"] for i in responses}
    for task in new_tasks:
        if "parent" in task:
            response = service.tasks().insert(tasklist=tasklist["id"], body=task).execute()
            service.tasks().move(
